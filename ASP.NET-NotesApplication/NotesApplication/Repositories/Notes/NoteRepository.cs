...
namespace NotesApplication.Repositories.Notes
{
    public class NoteRepository : INoteRepository
    {
        ...
        public async Task AddAsync(Note note)
        {
            ...
            int newId = await connection.QuerySingleAsync<int>(query, note);
            note.Id = newId;
        }
        ...
        public async Task<List<Note>> GetAllAsync(int userId, string? search = null, string? sortBy = "Created_At", bool descending = false, string? titleFilter = null)
        {
            using var connection = GetConnection();
            ...
            var notes = await connection.QueryAsync<Note>(query, new
            {
                UserId = userId,
                Search = $"%{search}%",  
